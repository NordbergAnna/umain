...
export const filterRestaurants = (
  ...
  if (foodCategories.length) {
    filtered = filtered.filter(
      (r) => r.filter_ids.some((id) => foodCategories.includes(id)) // Keep restaurant if it matches any selected category
    );
  }
  ...
  if (priceRanges.length) {
    filtered = filtered.filter(
      (r) => priceRanges.includes(r.price_range_id) // Keep restaurant if its price range matches a selected one
    );
  }
